...
import { DEFAULT_AWAY_COOLDOWN_MINUTES, DEFAULT_AWAY_COOLDOWN_HOURS, normalizeAwayText } from './auto-reply-policy.js';
...
export function enforceAwayPolicy(session = {}) {
  ...
  const awayText = normalizeAwayText(away.text);
  const enabled = away.enabled !== false;
  const minutes = Number(next.awayCooldownMinutes);

  next.away = { ...away, enabled, text: awayText };
  // Important owner alerts are a permanent safety/assistant feature.
  next.importantAlerts = true;
  next.awayCooldownMinutes = Number.isFinite(minutes) && minutes > 0
    ? Math.max(5, Math.min(60, minutes))
    : DEFAULT_AWAY_COOLDOWN_MINUTES;
  next.awayCooldownHours = next.awayCooldownMinutes / 60;
  next.afk = {
...
const sessionDefaults = () => enforceAwayPolicy({
  ...
  away: { enabled: true, text: '' }, awayCooldownMinutes: DEFAULT_AWAY_COOLDOWN_MINUTES, awayCooldownHours: DEFAULT_AWAY_COOLDOWN_HOURS,
  afk: { enabled: false, reason: '' }, stickerPack: { pack: 'A-X-HK', author: 'ABDULLAH-X-HK' },
  menuTheme: 'royal', ownerAlerts: true, importantAlerts: true, importantAlertCooldownMinutes: 30, dailyDrop: false, dailyDropHour: 18, dailyDropLastDate: '', autoCleanup: true, funVisuals: true,
...
const defaults = () => ({
  ...
  activity: [], commandAudit: [], importantAlerts: [], ownerMemory: {}, safetyDaily: {}, backups: []
});
  ...
  async init() {
    ...
    try {
      ...
      this.data = {
        ...
        importantAlerts: Array.isArray(parsed.importantAlerts) ? parsed.importantAlerts.slice(-100) : [],
        ownerMemory: parsed.ownerMemory && typeof parsed.ownerMemory === 'object' ? parsed.ownerMemory : {},
