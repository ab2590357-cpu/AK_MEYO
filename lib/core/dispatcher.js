...
export async function dispatchMessage(sock, msg, runtime = {}) {
  ...
  if (!parsed) {
    // Owner attention alerts are independent from Auto AI and Time Reply.
    // Run triage in parallel so the user's normal bot/AI reply is not delayed.
    maybeSendImportantAlert(ctx).catch((err) => logger.warn({ err, sessionId: ctx.sessionId }, 'Important owner alert failed'));
