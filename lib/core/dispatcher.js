...
import { AUTO_REPLY_TEXT, activeReplySlot, awayCooldownMs, normalizeAwayText, shouldSendAwayReply } from './auto-reply-policy.js';
import { ownerProfilePrompt } from './owner-profile.js';
import { botKnowledgePrompt } from './bot-knowledge.js';
import { maybeSendImportantAlert } from '../services/important-alerts.js';
import { localAssistantReply } from '../services/local-assistant.js';
import { sendCinematicIntro } from '../services/premium-experience.js';
...
function parseCommand(text, prefix) {
  ...
  if (name === 'menu' && ['dp', 'image', 'photo'].includes(lower[0])) {
    name = 'menudp';
    args = args.slice(1);
  } else if (name === 'active' && lower[0] === 'bot') {
    name = 'activebot';
    args = args.slice(1);
...
const scheduledReplyAt = new Map();
...
function aiPersonaPrompt(value = 'default') {
  ...
  const map = {
    ...
    professional: 'Use a natural professional-but-friendly tone. Match the sender’s formality; do not sound stiff or corporate in casual conversation.',
    friendly: 'Use a warm, friendly and natural conversational tone without overdoing emojis.',
    funny: 'Use a playful funny conversational tone. Keep humor harmless and avoid slurs, threats or cruel personal attacks.',
    savage: 'Use a sharp confident witty tone, but keep it non-hateful, non-threatening and not excessively abusive.',
    chill: 'Use a relaxed casual friendly tone, like a smart close friend.',
    serious: 'Use a calm serious direct tone with no unnecessary jokes.',
...
function directKey(ctx) {
  return `${ctx.sessionId}:${ctx.chat}`;
}

function rememberOwnerActivity(ctx, now = Date.now()) {
  if (!ctx.msg.key?.fromMe) return;
  const text = String(ctx.text || '').trim();
  if (ctx.prefix && text.startsWith(ctx.prefix)) return;
  ownerActivityAt.set(directKey(ctx), now);
  if (ownerActivityAt.size > 2000) ownerActivityAt.clear();
}
...
async function maybeDirectAutomation(ctx) {
  ...
  if (ctx.sessionId === 'main' && away?.enabled && slot) {
  ...
  const privateAI = ctx.sessionId === 'main' && !ctx.isGroup && Boolean(ctx.sessionSettings.autoAI);
  const groupAI = ctx.sessionId === 'main' && ctx.isGroup && Boolean(ctx.sessionSettings.autoAI) && wasMentioned;
  ...
  if (privateAI || groupAI) {
    ...
    const lastAIReply = aiReplyAt.get(key) || 0;
    if (now - lastAIReply < aiCooldownMs) return replied;
    ...
    try {
      ...
      const aiPrompt = [
        ...
        ownerProfilePrompt({ currentTime, availability }),
        botKnowledgePrompt(ctx.text),
        "Greeting rule: If the latest user message is a simple greeting such as hi, hello, hey or salam, introduce yourself naturally as A_X_HK AI Assistant, briefly mention Abdullah's current availability only when useful, and ask how you can help.",
        ...
        "Service rule: If someone asks about Abdullah's work or wants something built, fixed, automated, designed or developed, use the owner profile and helpfully ask for the minimum project details needed.",
      ...
      const aiReply = await askAI(aiPrompt, ctx.senderNumber, {
        temperature: 0.45,
        maxChars: 1600,
        systemPrompt: "You are A_X_HK AI Assistant, Abdullah's official WhatsApp AI assistant. Answer naturally, professionally but friendly, and match the user's language and tone. Use the supplied owner profile for Abdullah-specific facts. Never pretend to be Abdullah and never invent private owner information.",
      ...
      // Offline/local brain: if the external AI provider is unavailable, rate
      // limited or misconfigured, keep the assistant useful with deterministic
      // local intent handling instead of going silent.
      const localReply = localAssistantReply(ctx, { slot, currentTime });
      if (localReply) {
        aiReplyAt.set(key, now);
        await ctx.reply(localReply).catch(() => {});
        rememberChatMessage({
        ...
        replied = true;
      } else if (!replied && !ctx.isGroup) {
        aiReplyAt.set(key, now);
        await ctx.reply('Aap ka message receive ho gaya hai. Abdullah available honge to dekh lenge.').catch(() => {});
        replied = true;
      }
    }
  }
...
export async function dispatchMessage(sock, msg, runtime = {}) {
  ...
  // Private mode blocks public commands/features for non-owners, but the owner's
  // Auto AI and scheduled Time Reply must keep working as independent inbox automations.
  const effectiveMode = ctx.sessionSettings.mode || config.mode;

  // One-time public cinematic welcome. Stored per session/user so it never
  // becomes a repetitive auto-reply.
  if (effectiveMode !== 'private' && !ctx.isGroup && !ctx.isOwner && !ctx.user.axhkIntroShown) {
    ctx.user.axhkIntroShown = true;
    ctx.user.axhkJoinedAt ||= new Intl.DateTimeFormat('en-GB', { timeZone: config.timezone, dateStyle: 'medium' }).format(new Date());
    await db.save();
    try { await sendCinematicIntro(ctx); }
    catch (err) { logger.warn({ err, sessionId: ctx.sessionId }, 'A_X_HK cinematic intro skipped'); }
  }

  if (effectiveMode === 'private' && !ctx.isOwner) {
    if (!ctx.text) return;
    const privateParsed = parseCommand(ctx.text, ctx.prefix);
    if (privateParsed) return;

    // Important-message triage stays active even while the bot is private.
    try {
      if (await maybeSendImportantAlert(ctx)) return;
    } catch (err) {
      logger.warn({ err, sessionId: ctx.sessionId }, 'Important owner alert failed in private mode');
    }
  ...
  if (!parsed) {
    // Important owner notes take priority over generic Auto AI replies so the
    // user gets a clear acknowledgement and Abdullah receives the alert.
    try {
      if (await maybeSendImportantAlert(ctx)) return;
    } catch (err) {
      logger.warn({ err, sessionId: ctx.sessionId }, 'Important owner alert failed');
    }

    // In private chats, Auto AI ON means the AI owns every normal incoming
    // message. Legacy exact auto-replies must not intercept those messages.
    if (ctx.sessionId === 'main' && !ctx.isGroup && ctx.sessionSettings.autoAI) {
      await maybeDirectAutomation(ctx);
      return;
    }
  ...
  // Exact registered commands always win over category shortcuts.
  // Example: .owner must open the owner card, not the "owner" menu category.
  if (!parsed.argText && !getCommand(parsed.name)) {
  ...
  if (command.masterOnly && !ctx.isMasterOwnerAction) { await blockCommand('master owner only command'); return; }
