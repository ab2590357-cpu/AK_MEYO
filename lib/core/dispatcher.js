...
async function maybeDirectAutomation(ctx) {
  ...
  if (privateAI || groupAI) {
    try {
      ...
      if (!replied && !ctx.isGroup) {
        await ctx.reply([
          '🤖 *A_X_HK AI TEMPORARILY BUSY*',
          '',
          'Aap ka message receive ho gaya hai.',
          'AI provider abhi rate-limit/busy hai, is liye smart reply temporarily available nahi.',
          'Thori dair baad dobara message karein — bot automatically reply karega.'
        ].join('\n')).catch(() => {});
        replied = true;
      }
    }
