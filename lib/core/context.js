...
export async function buildContext(sock, msg, runtime = {}) {
  ...
  const peerSender = await resolvePhoneJid(sock, rawSender, alternateSender);

  const rawBotJid = cleanJid(sock.user?.id || '');
  const botJid = await resolvePhoneJid(sock, rawBotJid, sock.user?.lid || '');

  // In a direct chat, remoteJid is the other person even when the linked
  // account itself typed the command. Treat fromMe as the actual command actor.
  // This keeps identity/pass/user data bound to the person who sent the command,
  // not to whichever chat window the command was typed inside.
  const sender = msg.key?.fromMe ? botJid : peerSender;
  const senderNumber = bareNumber(sender);
  const botNumber = bareNumber(botJid);
  const botLid = cleanJid(sock.user?.lid || '');
  const directJids = [rawSender, alternateSender, peerSender].map(cleanJid).filter(Boolean);
  ...
  const isMasterOwnerAction = Boolean(isMasterOwner || (sessionId === 'main' && isLinkedOwnerAction));
  ...
  return {
    ...
    isOwner: owner, isMasterOwnerAction, isSelfChat, isLinkedOwnerAction, text, prefix, sessionId, sessionSettings,
