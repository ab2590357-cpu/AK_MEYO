...
export const DEFAULT_AWAY_COOLDOWN_MINUTES = Math.max(5, Math.min(60, Number(process.env.AUTO_REPLY_COOLDOWN_MINUTES || process.env.AWAY_COOLDOWN_MINUTES || 5)));
...
export function shouldSendAwayReply({
  ...
  if (away?.enabled === false) return false;
