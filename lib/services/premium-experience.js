...
export async function sendControlCenter(ctx) {
  const cards = [
    { title: 'AI CORE', body: 'Ask, think and create with A_X_HK AI.', buttons: [{ id: `${ctx.prefix}ai hello`, text: 'AI' }, { id: `${ctx.prefix}vibe`, text: 'Vibe' }] },
    { title: 'MAGIC', body: 'Reply to a message and transform it.', buttons: [{ id: `${ctx.prefix}magic`, text: 'Magic' }, { id: `${ctx.prefix}meme`, text: 'Meme' }] },
    { title: 'IDENTITY', body: 'Premium member pass and digital ID.', buttons: [{ id: `${ctx.prefix}pass`, text: 'Pass' }, { id: `${ctx.prefix}id`, text: 'ID' }] },
    { title: 'STUDIO', body: 'Poster, DP and sticker creation.', buttons: [{ id: `${ctx.prefix}poster A_X_HK`, text: 'Poster' }, { id: `${ctx.prefix}dp`, text: 'DP Studio' }] },
    { title: 'MUSIC', body: 'Search, play and browse music.', buttons: [{ id: `${ctx.prefix}song Pasoori`, text: 'Song' }, { id: `${ctx.prefix}playlist chill`, text: 'Playlist' }] },
    { title: 'SECRET', body: 'Hidden A_X_HK experience layer.', buttons: [{ id: `${ctx.prefix}secret`, text: 'Unlock' }, { id: `${ctx.prefix}box`, text: 'Mystery' }] }
  ];

  // Always send a normal media card first. Native-flow carousels can be
  // accepted by WhatsApp but not rendered in some clients/self-chat.
  await sendCard(ctx, {
    kicker: 'A_X_HK • PREMIUM SYSTEM',
    title: 'CONTROL CENTER',
    subtitle: 'AI • MAGIC • IDENTITY • STUDIO • MUSIC',
    badge: 'AX',
    lines: [
      `AI CORE     •  ${ctx.prefix}ai  •  ${ctx.prefix}vibe`,
      `MAGIC LAB   •  ${ctx.prefix}magic  •  ${ctx.prefix}meme`,
      `IDENTITY    •  ${ctx.prefix}pass  •  ${ctx.prefix}id`,
      `STUDIO      •  ${ctx.prefix}poster  •  ${ctx.prefix}dp`,
      `MUSIC DECK  •  ${ctx.prefix}song  •  ${ctx.prefix}playlist`,
      `SECRET      •  ${ctx.prefix}secret  •  ${ctx.prefix}box`,
      '',
      `FULL MENU   •  ${ctx.prefix}menu all`
    ]
  }, { caption: `⚡ *A_X_HK CONTROL CENTER*\nPremium command hub online.\n\n${config.footerMessage}` });

  // Best-effort enhanced carousel after the guaranteed visible card.
  try {
    await sendInteractiveHub(ctx, {
      title: 'A_X_HK CONTROL CENTER',
      body: 'Swipe the cards or use the commands shown.',
      cards
    });
  } catch {}
  return true;
...
export async function sendAxhkOs(ctx) {
  const cards = [
    { title: 'NEURAL AI', body: 'Ask AI or scan your vibe.', buttons: [{ id: `${ctx.prefix}ai what can you do`, text: 'Ask AI' }, { id: `${ctx.prefix}vibe`, text: 'Vibe Scan' }] },
    { title: 'MAGIC LAB', body: 'Transform a quoted message.', buttons: [{ id: `${ctx.prefix}magic`, text: 'Magic' }, { id: `${ctx.prefix}meme`, text: 'Meme' }] },
    { title: 'IDENTITY', body: 'Your A_X_HK member identity.', buttons: [{ id: `${ctx.prefix}pass`, text: 'Pass' }, { id: `${ctx.prefix}id`, text: 'Digital ID' }] },
    { title: 'VISUAL STUDIO', body: 'Create posters and profile visuals.', buttons: [{ id: `${ctx.prefix}poster A_X_HK`, text: 'Poster' }, { id: `${ctx.prefix}dp`, text: 'DP Studio' }] },
    { title: 'MUSIC DECK', body: 'Play songs and browse playlists.', buttons: [{ id: `${ctx.prefix}music Pasoori`, text: 'Music' }, { id: `${ctx.prefix}playlist trending`, text: 'Playlist' }] },
    { title: 'HIDDEN LAYER', body: 'Secret menu, story and mystery box.', buttons: [{ id: `${ctx.prefix}secret`, text: 'Secret' }, { id: `${ctx.prefix}story`, text: 'Story' }, { id: `${ctx.prefix}box`, text: 'Box' }] }
  ];

  await sendCard(ctx, {
    kicker: 'A_X_HK • PRIVATE DIGITAL EXPERIENCE',
    title: 'A_X_HK OS',
    subtitle: 'PREMIUM WHATSAPP EXPERIENCE',
    badge: 'OS',
    lines: [
      'NEURAL AI • MAGIC LAB • IDENTITY',
      'VISUAL STUDIO • MUSIC DECK',
      'STORY MODE • SECRET LAYER',
      '',
      `CONTROL CENTER  •  ${ctx.prefix}menu`,
      `SECRET LAYER    •  ${ctx.prefix}secret`,
      `OWNER PROFILE   •  ${ctx.prefix}owner`
    ]
  }, { caption: `✦ *A_X_HK OS ONLINE*\nYour premium WhatsApp layer is ready.\n\n${config.footerMessage}` });

  try {
    await sendInteractiveHub(ctx, {
      title: 'A_X_HK OS',
      body: 'Welcome to A_X_HK OS — AI, media, identity, music and creative tools inside WhatsApp.',
      cards
    });
  } catch {}
  return true;
