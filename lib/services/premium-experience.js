...
export async function renderDpStudio(buffer, { label = 'A_X_HK', vip = false } = {}) {
  ...
  const mask = Buffer.from(`<svg width="${size}" height="${size}" xmlns="http://www.w3.org/2000/svg">
    <circle cx="540" cy="500" r="425" fill="white"/>
  </svg>`);
...
export async function sendCard(ctx, opts = {}, { caption = '' } = {}) {
  ...
  return ctx.send({ image, caption: caption || opts.caption || '' }, { quoted: ctx.msg });
}
...
export async function sendControlCenter(ctx) {
  ...
  return sendInteractiveHub(ctx, {
    title: 'A_X_HK CONTROL CENTER',
    body: 'A premium WhatsApp command center. Swipe the cards or use the commands shown.',
    cards
  });
}
