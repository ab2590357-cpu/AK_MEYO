...
function premiumTextCard(opts = {}, extra = '') {
  const kicker = clean(opts.kicker || 'A_X_HK', 64);
  const title = clean(opts.title || 'PREMIUM CARD', 64);
  const subtitle = clean(opts.subtitle || '', 120);
  const body = (Array.isArray(opts.lines) ? opts.lines : [])
  ...
  if (cleanExtra) rows.push('', cleanExtra);
  if (config.footerMessage) rows.push('', config.footerMessage);
  return rows.join('\n');
}

export async function sendCard(ctx, opts = {}, { caption = '' } = {}) {
  return ctx.reply(premiumTextCard(opts, caption || opts.caption || ''));
}
...
export async function sendOwnerCard(ctx, profile = {}) {
  const name = profile.name || config.ownerName;
  const brand = profile.brandLong || config.ownerName;
  const services = (profile.services || []).slice(0, 4).map((s) => clean(s, 46));
  const rows = [
    '╭━━━━〔 👑 *A_X_HK OWNER PROFILE* 👑 〕━━━━╮',
    `┃ ✦ *${brand}*`,
    '┃ _VERIFIED OWNER • BUILDER • AUTOMATION & AI_',
    '┣━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┫',
    `┃ 👤 *Name*      • ${name}`,
    ...
    ...services.map((s) => `┃   ◆ ${s}`),
    '┣━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┫',
    `┃ 📲 *Contact*   • ${config.ownerContactUrl || 'Not configured'}`,
    `┃ ⚡ *Bot*       • ${config.shortName} V${config.version}`,
    '╰━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━╯',
    '',
...
export async function sendAIAnswerCard(ctx, answer = '', question = '') {
  const q = clean(question, 100) || 'AI RESPONSE';
  const safeAnswer = String(answer || '').trim().slice(0, 3300);
  return ctx.reply([
    '╭━━━〔 🧠 *A_X_HK AI* 〕━━━╮',
    `┃ *NEURAL RESPONSE*`,
    `┃ _${q}_`,
