...
function explicitOwnerNotice(text = '') {
  const value = String(text || '').toLowerCase();
  return /\b(note\s*(?:kar|kr)\s*(?:lo|lena|len|dena)|yaad\s*rakh|bata\s*(?:dena|do|daina)|zaroor\s*bata|lazmi\s*bata|must\s*(?:tell|inform)|tell\s*abdullah|inform\s*abdullah|important\s*(?:hai|baat)|boh?ut\s+important)\b/i.test(value)
    && /\b(abdullah|owner|important|zaroor|lazmi|note)\b/i.test(value);
}
...
function signalScore(text = '') {
  ...
  hit(/\b(urgent|asap|emergency|immediately|today|jaldi|foran|zaroor|lazmi|important|bohut important|bohot important)\b/i, 3);
  hit(/\b(note\s*(?:kar|kr)|yaad\s*rakh|bata\s*(?:dena|do|daina)|must\s*(?:tell|inform)|tell\s*abdullah|inform\s*abdullah)\b/i, 4);
...
function shouldSkipDuplicate(ctx, classification, now = Date.now()) {
  if (explicitOwnerNotice(ctx.text)) return false;
...
async function classifyImportant(ctx) {
  const directNotice = explicitOwnerNotice(ctx.text);
  ...
  if (directNotice) {
    return {
      important: true,
      priority: 'high',
      reason: 'User explicitly asked to note this and tell Abdullah',
      summary: compact(ctx.text, 240)
    };
  }
...
export async function maybeSendImportantAlert(ctx) {
  ...
  try {
    await ctx.reply([
      ...
      '',
      'Samajh gaya — ye important baat maine note kar li hai.',
      'Abdullah ko owner inbox mein alert bhej diya hai. Jab woh available honge, ye message unke important alerts mein rahega.'
    ].join('\n'));
  } catch (err) {
    logger.warn({ err, sessionId: ctx.sessionId }, 'Important alert acknowledgement failed');
  }
