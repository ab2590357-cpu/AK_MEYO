...
const QUOTA_BLOCK_MS = 30 * 60 * 1000;
...
function modelFailoverError(err) {
  const status = Number(err?.status || 0);
  const detail = String(err?.detail || err?.message || '');
  if (modelRouteError(status, detail)) return true;
  return status === 429 && /rate[_ -]?limit|rate_limit_exceeded|requests per (?:min|minute|day)|\bRPM\b|\bRPD\b/i.test(detail);
}

function quotaExhaustedError(err) {
  const status = Number(err?.status || 0);
  const detail = String(err?.detail || err?.message || '');
  return status === 429 && /insufficient[_ -]?quota|credit[_ -]?balance[_ -]?exhausted|no credits remaining|billing hard limit|quota exceeded/i.test(detail);
}

function transientError(err) {
  const status = Number(err?.status || 0);
  if (modelFailoverError(err)) return false;
  if (['AbortError', 'TimeoutError'].includes(String(err?.name || ''))) return true;
  if (quotaExhaustedError(err)) return false;
...
export function getAIStatus() {
  return {
    ...
    quotaBlockedUntil: quotaBlockedUntil > Date.now() ? new Date(quotaBlockedUntil).toISOString() : null,
    quotaBlockedReason: quotaBlockedUntil > Date.now() ? quotaBlockedReason : '',
    ...runtime
  };
}
...
async function chatWithRetry(model, prompt, user, options) {
  ...
  for (let attempt = 0; attempt <= retryLimit; attempt += 1) {
    try {
      ...
      lastError = err;
      if (quotaExhaustedError(err)) throw err;
      if (modelFailoverError(err)) throw err;
...
export async function askAI(prompt, user = 'whatsapp-user', options = {}) {
  ...
  if (quotaBlockedUntil && quotaBlockedUntil <= Date.now()) {
    quotaBlockedUntil = 0;
    quotaBlockedReason = '';
  }
  if (quotaBlockedUntil > Date.now()) {
    const err = new Error('AI provider credits/quota are temporarily unavailable. Local fallback should handle this message.');
    err.status = 429;
    err.code = 'AI_PROVIDER_QUOTA_BLOCKED';
    err.detail = quotaBlockedReason || 'Provider quota is temporarily blocked.';
    throw err;
  }
  ...
  for (let index = 0; index < candidates.length; index += 1) {
    ...
    try {
      ...
      if (quotaExhaustedError(err)) {
        quotaBlockedUntil = Date.now() + QUOTA_BLOCK_MS;
        quotaBlockedReason = truncate(err?.detail || err?.message || 'Provider quota exhausted.', 300);
        recordFailure(err);
        throw err;
      }

      if (!modelFailoverError(err)) {
        recordFailure(err);
        throw err;
      }
      markBadModel(model);
