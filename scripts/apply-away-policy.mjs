...
const NEW_BLOCK = [
  ...
  '    autoReplyAt.set(key, now);',
  "    const text = `📨 ${away.text || 'I am currently away.'}`;",
  '    await ctx.reply(`${text}\\n\\n${config.footerMessage}`);',
  '    return true;',
  '  }'
].join('\n');
...
export function patchDispatcherSource(source) {
  ...
  if (hasOldAfkReply || hasOldCooldown || !hasAwayPolicy) {
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  applyAwayPolicy()
    .then((changed) => {
      console.log(changed ? 'A-X-HK away policy applied' : 'A-X-HK away policy already applied');
    })
    .catch((error) => {
